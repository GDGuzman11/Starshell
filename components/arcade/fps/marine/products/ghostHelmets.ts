...
const ghostwalker: Build = (spec, rt) => {
  ...
  g.add(cylY(0.012, 0.22 * b, dark, 0.02 * b, 0.24 * b, -0.04)); // deployable sensor mast
  gl(cylY(0.02, 0.03, glow, 0.02 * b, 0.36 * b, -0.04)); // mast beacon
...
const nightwatch: Build = (spec, rt) => {
  ...
  g.add(box(0.21 * b, 0.24 * b, 0.22 * b, body, 0, 0.02, 0)); // compact shell
  g.add(box(0.23 * b, 0.05 * b, 0.05, dark, 0, 0.12 * b, 0.02)); // shell cap
...
const vector: Build = (spec, rt) => {
  const { g, b, body, dark, glow, gl, moving } = mk(spec, rt);
  const head = cylY(0.14 * b, 0.28 * b, body, 0, 0.02, 0, 6); head.rotation.y = Math.PI / 6; g.add(head); // hex faceted head
...
const cipher: Build = (spec, rt) => {
  const { g, b, body, dark, glow, gl, moving } = mk(spec, rt);
  g.add(capsuleY(0.13 * b, 0.1 * b, body, 0, 0.02, 0)); // minimal rounded skull
...
const wraith: Build = (spec, rt) => {
  const { g, b, body, dark, glow, gl } = mk(spec, rt);
  g.add(box(0.19 * b, 0.22 * b, 0.24 * b, body, 0, 0.0, 0)); // low skull
