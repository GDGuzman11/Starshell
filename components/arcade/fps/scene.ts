...
export function buildWorld(level: Level3D, tier: RenderTier = 'desktop'): World {
  ...
  // Only STATIONS get a static marker here (they're fixed structures). Ammo/shield/
  // health pickups are dynamic (they relocate on pickup) and are drawn + moved by
  // useFpsLoop with their own symbol meshes.
  for (const m of level.modules ?? []) {
    if (m.kind !== 'station') continue;
    const mat = new THREE.MeshBasicMaterial({ color: 0xe0ffe6, transparent: true, opacity: 0.9, blending: THREE.AdditiveBlending, depthWrite: false });
    const ringGeo = new THREE.TorusGeometry(0.7, 0.13, 8, 20);
    const ring = new THREE.Mesh(ringGeo, mat);
    ring.position.set(m.cx, 5.9, m.cz);
